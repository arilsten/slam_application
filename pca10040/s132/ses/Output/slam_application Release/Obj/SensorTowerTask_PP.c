...
# 2 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam_applicationLeithe\\software\\SensorTowerTask.c" 2
...
int *preparePoseMessage(uint16_t x, uint16_t y, uint16_t theta, uint8_t servoAngle, uint16_t* sensorData){
    ...
    for(int i = 0; i<=4; i++){
        if(sensorData[i] < DETECTION_THRESHOLD){
            xObject = xhat - cos(thetahat)*46 + cos(thetahat+(servoAngle+i*90)*
# 42 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam_applicationLeithe\\software\\SensorTowerTask.c" 3 4
                                                                                                    3.14159265358979323846 
# 42 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam_applicationLeithe\\software\\SensorTowerTask.c"
                                                                                                    / 180.0)*sensorData[i];
            yObject = yhat - cos(thetahat)*46 + sin(thetahat+(servoAngle+i*90)*
# 43 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam_applicationLeithe\\software\\SensorTowerTask.c" 3 4
                                                                                                    3.14159265358979323846 
# 43 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam_applicationLeithe\\software\\SensorTowerTask.c"
                                                                                                    / 180.0)*sensorData[i];
            data[i*4+6] = (xObject >> 8);
            data[i*4+7] = (xObject & 0xff);
            data[i*4+8] = (yObject >> 8);
...
# 150 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam_applicationLeithe\\software\\SensorTowerTask.c"
            ...
            uint8_t sendData[23];
            sendData = preparePoseMessage(xhat, yhat, thetahat, servoStep, sensor16);
            i2cSendNOADDR(0x72, sendData, 23);
# 197 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam_applicationLeithe\\software\\SensorTowerTask.c"
          askWaypointCounter++;
          if(askWaypointCounter>20);
            int16_t oldwaypoint[2];
            oldwaypoint[0]=waypoint[0];
            oldwaypoint[1]=waypoint[1];
            uint8_t tempwaypoint[5];

            i2cReciveNOADDR(0x72,&tempwaypoint,5);

            waypoint[0] = *((int16_t*)&tempwaypoint[1]);
            waypoint[1] = *((int16_t*)&tempwaypoint[3]);
              ...
              if(oldwaypoint[0] != waypoint[0] || oldwaypoint[1] != waypoint[1]){
                  ...
                  if (1 && (3 >= NRF_LOG_SEVERITY_INFO) && (NRF_LOG_SEVERITY_INFO <= 3)) { if (NRF_LOG_SEVERITY_DEBUG >= NRF_LOG_SEVERITY_INFO) { nrf_log_frontend_std_0(((NRF_LOG_SEVERITY_INFO) | m_nrf_log_app_logs_data_dynamic.module_id << 16), "SEND SETPOINT FROM"); } };
              }
            askWaypointCounter =0;
            }
# 238 "C:\\nRF5_SDK_15.0.0_a53641a\\examples\\ble_peripheral\\slam_applicationLeithe\\software\\SensorTowerTask.c"
            if ((servoStep * servoResolution <= 90) && (rotationDirection == 4) && (robotMovement < 3)) {
                servoStep++;
            } else if ((servoStep * servoResolution > 0) && (rotationDirection == 3) && (robotMovement < 3)) {
                servoStep--;
            }

            if ((servoStep * servoResolution >= 90) && (rotationDirection == 4)) {
                rotationDirection = 3;
            } else if ((servoStep * servoResolution <= 0) && (rotationDirection == 3)) {
                rotationDirection = 4;
            }
        }
