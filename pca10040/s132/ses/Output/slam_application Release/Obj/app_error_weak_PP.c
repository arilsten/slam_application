...
__attribute__((weak)) void app_error_fault_handler(uint32_t id, uint32_t pc, uint32_t info)
{
    ...
    if (1 && (3 >= NRF_LOG_SEVERITY_ERROR) && (NRF_LOG_SEVERITY_ERROR <= 3)) { if (NRF_LOG_SEVERITY_DEBUG >= NRF_LOG_SEVERITY_ERROR) { nrf_log_frontend_std_0(((NRF_LOG_SEVERITY_ERROR) | m_nrf_log_app_logs_data_dynamic.module_id << 16), "Fatal error"); } };
# 99 "C:\\nRF5_SDK_15.0.0_a53641a\\components\\libraries\\util\\app_error_weak.c"
    do { if (((CoreDebug_Type *) (0xE000EDF0UL))->DHCSR & (1UL )) { __asm__("BKPT 0");; } }while (0);



    if (1 && (3 >= NRF_LOG_SEVERITY_WARNING) && (NRF_LOG_SEVERITY_WARNING <= 3)) { if (NRF_LOG_SEVERITY_DEBUG >= NRF_LOG_SEVERITY_WARNING) { nrf_log_frontend_std_0(((NRF_LOG_SEVERITY_WARNING) | m_nrf_log_app_logs_data_dynamic.module_id << 16), "System reset"); } };
    NVIC_SystemReset();
